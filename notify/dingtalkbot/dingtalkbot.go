...
// Notifier implements a Notifier for generic dingtalkbot.
type Notifier struct {
	conf    *config.DingTalkBotConfig
...
func New(conf *config.DingTalkBotConfig, t *template.Template, l log.Logger, httpOpts ...commoncfg.HTTPClientOption) (*Notifier, error) {
	client, err := commoncfg.NewClientFromConfig(*conf.HTTPConfig, "dingtalkbot", httpOpts...)
	if err != nil {
		return nil, err
	}
	return &Notifier{
		...
		logger: l,
		client: client,
		// dingtalkbot are assumed to respond with 2xx response codes on a successful
...
func (n *Notifier) Notify(ctx context.Context, alerts ...*types.Alert) (bool, error) {
	...
	for _, alert := range alerts {
		dingtalkMsg += alert.Annotations.String()
	}
	dtmsg := &DingTalkMessage{
		Alerts: dingtalkMsg,
	}
