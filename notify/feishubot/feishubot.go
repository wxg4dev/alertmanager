...
// Notifier implements a Notifier for generic feishubot.
...
func New(conf *config.FeiShuBotConfig, t *template.Template, l log.Logger, httpOpts ...commoncfg.HTTPClientOption) (*Notifier, error) {
	client, err := commoncfg.NewClientFromConfig(*conf.HTTPConfig, "feishubot", httpOpts...)
	if err != nil {
		return nil, err
	}
	return &Notifier{
		...
		logger: l,
		client: client,
		// feishubot are assumed to respond with 2xx response codes on a successful
...
type Message struct {
	...
	Content         FeiShuMessage `json:"content"`
...
func (n *Notifier) Notify(ctx context.Context, alerts ...*types.Alert) (bool, error) {
	...
	for _, alert := range alerts {
		feishuMsg += alert.Annotations.String()
	}
	fsmsg := &FeiShuMessage{
		Alerts: feishuMsg,
	}
